...
import { getEmployeeColumns } from "@/components/table/columns/employee-coulmn";
...
export default function EmployeePage() {
  ...
  const { data, isLoading, isError, error, isFetching } = useQuery<Employee[]>({
    queryKey: ["employees"],
  ...
  const handleBulkDelete = useCallback(
    async (selectedEmployees: Employee[]) => {
      ...
      if (hasFailure) {
        toast.error("Failed to delete some employees. Please try again.");
      } else {
        toast.success(
      ...
      <Card>
          ...
          <div className="flex flex-row justify-between">
            <div>
              <p className="text-sm text-muted-foreground">
                Filter, sort, and manage employee records in one place.
              </p>
            </div>
        ...
        <CardContent className="space-y-4">
          <AdvancedTable<Employee>
            columns={columns}
            data={employees}
            searchPlaceholder="employees"
            searchColumn="first_name"
            onDeleteSelected={handleBulkDelete}
            isClickable={false}
            isLoading={isFetching}
            paginationKey="employees_page"
            pageSizeKey="employees_page_size"
